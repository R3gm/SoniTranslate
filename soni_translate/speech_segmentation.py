...
def align_speech(audio, result):
    ...
    - result (dict): Updated metadata after aligning the segments with the audio.
      This includes character-level alignments if 'return_char_alignments' is set to True.

    Notes:
    - This function uses language-specific models to align speech segments.
    - It performs language compatibility checks and selects the appropriate alignment model.
...
def diarize_speech(audio_wav, result, min_speakers, max_speakers, YOUR_HF_TOKEN, model_name="pyannote/speaker-diarization@2.1"):
    ...
    - model_name (str): Name of the speaker diarization model to be used (default: "pyannote/speaker-diarization@2.1").
    ...
    - If only one speaker is specified, each segment is automatically assigned as the first speaker, eliminating the need for diarization inference.
    """
        ...
        try:
            ...
            if "'NoneType' object has no attribute 'to'" in error:
                if model_name == diarization_models["pyannote_2.1"]:
                    ...
                    raise ValueError("New Licence Pyannote 3.1: You need to have an account on Hugging Face and accept the license to use the models: https://huggingface.co/pyannote/speaker-diarization-3.0 and https://huggingface.co/pyannote/segmentation-3.0.")
            else:
                raise error
